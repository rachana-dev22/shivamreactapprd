...
export default function ServerBrowser() {
  ...
  const apiKey = "rAYLJE3GoTaY";
  const apiKeySecret = "F2mWdX0rnNbtexkIMPCMxGwqnDl3cdJx";
  const userID = "8affcdbc16fc4910acb8a6dc268cd7ed";
  const workerUrl = "https://edutools.eastus.cloudapp.azure.com";
  ...
  const findAndJoinServer = async () => {
    ...
    if (availableServer) {
      try {
        ...
        const fullUrl = `https://edutools.eastus.cloudapp.azure.com${joinUrl}`;
  ...
  const waitForSessionToRun = async (share_id) => {
    ...
    while (status !== "running") {
      try {
        const statusResponse = await axios.get(`${workerUrl}/api/public/get_kasm_status`, {
          params: {
            api_key: apiKey,
            api_key_secret: apiKeySecret,
            share_id: share_id,
